...
\begin{codeblock}
template<class T>
  inline constexpr bool @\placeholder{is-integer-like}@ = @\seebelow@;     // exposition only

template<class T>
  inline constexpr bool @\placeholder{is-signed-integer-like}@ = @\seebelow@; // exposition only
...
otherwise, it is an \defn{unsigned-integer-class}{type}.
...
\item \tcode{iterator_traits<X>::difference_type} is a signed integer type or void, and
...
\begin{codeblock}
return static_cast<iter_difference_t<R>>(ranges::size(r));      // \ref{range.prim.size}
\end{codeblock}
