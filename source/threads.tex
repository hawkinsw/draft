...
\pnum
\effects
Initializes \tcode{callback} with \tcode{static_cast<C\&\&>(cb)}.
...
such that \tcode{static_cast<Callback\&\&>(callback)()}
...
\pnum
\requires
\tcode{F} and each $\tcode{T}_i$ in \tcode{Args} shall satisfy the
\oldconcept{MoveConstructible} requirements.
\tcode{%
        \placeholdernc{INVOKE}(\brk{}%
        \placeholdernc{decay-copy}(\brk{}%
...
\end{note}
If the \placeholdernc{INVOKE} expression exits via an exception,
