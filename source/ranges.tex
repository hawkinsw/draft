...
\item
  Otherwise, \tcode{\placeholder{make-unsigned-like}(ranges::end(E) - ranges::begin(E))}\iref{range.subrange}
...
namespace std::ranges {
  ...
  private:
    ...
    I begin_ = I();                                        // \expos
    S end_ = S();                                          // \expos
    @\placeholder{make-unsigned-like-t}@(iter_difference_t<I>) size_ = 0;  // \expos; present only
  ...
  public:
    ...
    constexpr subrange(I i, S s, @\placeholder{make-unsigned-like-t}@(iter_difference_t<I>) n)
  ...
  template<Iterator I, Sentinel<I> S>
    subrange(I, S, @\placeholder{make-unsigned-like-t}@(iter_difference_t<I>)) ->
  ...
  template<@\placeholder{iterator-sentinel-pair}@ P>
    subrange(P, @\placeholder{make-unsigned-like-t}@(iter_difference_t<tuple_element_t<0, P>>)) ->
  ...
  template<@\placeholder{forwarding-range}@ R>
    subrange(R&&, @\placeholder{make-unsigned-like-t}@(iter_difference_t<iterator_t<R>>)) ->
...
\tcode{n == \placeholder{make-unsigned-like}(ranges::distance(i, s))}.
...
\item Otherwise, equivalent to: \tcode{return \placeholder{make-unsigned-like-t}(end_ - begin_);}
\end{itemize}
\end{itemdescr}
...
if (d >= 0)
  size_ -= @\placeholder{make-unsigned-like}@(d);
else
  size_ += @\placeholder{make-unsigned-like}@(-d);
return *this;
\end{codeblock}
