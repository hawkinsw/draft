...
template<class F, class... Args>
  invoke_result_t<F, Args...> invoke(F&& f, Args&&... args)
    constexpr noexcept(is_nothrow_invocable_v<F, Args...>);
\end{itemdecl}
...
\begin{codeblock}
@\placeholdernc{INVOKE}@(static_cast<@$\tcode{V}_fd$@>(@$\tcode{v}_fd$@),
       static_cast<@$\tcode{V}_1$@>(@$\tcode{v}_1$@), static_cast<@$\tcode{V}_2$@>(@$\tcode{v}_2$@), @$\dotsc$@, static_cast<@$\tcode{V}_N$@>(@$\tcode{v}_N$@))
\end{codeblock}
for the first overload, and
\begin{codeblock}
@\placeholdernc{INVOKE}@<R>(static_cast<@$\tcode{V}_fd$@>(@$\tcode{v}_fd$@),
          static_cast<@$\tcode{V}_1$@>(@$\tcode{v}_1$@), static_cast<@$\tcode{V}_2$@>(@$\tcode{v}_2$@), @$\dotsc$@, static_cast<@$\tcode{V}_N$@>(@$\tcode{v}_N$@))
\end{codeblock}
for the second overload,
where the values and types of the target argument @$\tcode{v}_fd$@ and
...
\tcode{static_cast<$\tcode{TD}_i$ \cv{} \&>($\tcode{td}_i$)(\brk{}std::forward<$\tcode{U}_j$>($\tcode{u}_j$)...)}
...
\pnum
The value of the target argument $\tcode{v}_fd$ is \tcode{fd} and
its corresponding type $\tcode{V}_fd$ is \tcode{FD \cv{} \&}.
\indexlibrary{\idxcode{bind}|)}%
